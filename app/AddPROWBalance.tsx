...
const PreOrderWalletBalance = () => {
  const paystackWebViewRef = useRef(paystackProps.PayStackRef); 
  const [amountToPaid, setAmountToPaid] = useState('');
  const [prowBalance, setProwBalance] = useState(0);
  ...
  .then(
    (result) => {
     ...
     // console.log(result[0].balance);
      ...
      setProwBalance(result[0].balance);
  },
    (error) => {
      console.log(error);
    }
...
const handlePaystackResponseCancel = () =>{
  ...
  const handlePaystackResponseSuccess = async () =>{
    ...
    if(parseFloat(amountToPaid) < 1000){
      alert("Minimum deposit is N1,000"); 
      return;
    }
  ...
  return (
    ...
    <View style={tw`flex-1 mt-20 bg-gray-100`}>
      ...
      />
        ...
        <ProfileHeadInfo />
        <PROWBalance balance={prowBalance} />
      </Animated.View>
      ...
      <View style={tw`flex-row items-center mb-4`}>
        ...
        <Text style={tw`ml-2 text-lg font-bold`}>Load Your Pre Order Wallet </Text>
      </View>
...
export default PreOrderWalletBalance;
