  ...
  try {
    ...
    cookieHeader.split(";").forEach((cookie) => {
      ...
      if (trimmedCookie.includes("=")) {
        ...
        ///initiate the cookie entry if it doesnt exist
    ...
    try {
      ...
      if (cookies[COOKIE_NAME].maxAge) {
        ...
        //We don't know excatly when it was set, but we can estimate
        //using the token's iat (issued at) claim if available
        const issuedAt =
          (verified.payload.iat as number) || Math.floor(Date.now() / 100);
